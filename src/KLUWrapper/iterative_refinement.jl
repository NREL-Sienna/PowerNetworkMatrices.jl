...
The cache must already be factored (`is_factored(cache) == true`). The
function does not mutate `cache`'s factor; it only triggers in-place
`solve!` calls on a residual buffer.
